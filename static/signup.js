document.addEventListener("DOMContentLoaded", () => {
    ...
    const usernameRules = "The username must only consist of alphanumeric characters. Usernames are case sensitive. Pick the username wisely as it can't be changed later. A username can't be longer than 20 characters."
    ...
    username.addEventListener('focusout', () => {
        if (!username.value.match(userRegex) || username.value.length > 20) {
            document.getElementById("username_warning").innerHTML = usernameRules;
            verifications[0] = 0;
        }
    });
    ...
    username.addEventListener('keyup', () => {
        ...
        if (username.value.match(userRegex) && username.value.length < 21) {
